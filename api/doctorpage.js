...
app.post("/", urlencodedParser, (req, res, next) => {
  ...
  for (var i = 0; i < obj.doctors.length; i++) {
    console.log(obj.doctors[i].Docname);
    if (
      obj.doctors[i].Docname === req.body.uname &&
      obj.doctors[i].docid === req.body.docid
    ) {
  ...
  if (count == 0) {
    ...
    const yourWebHookURL =
      "https://hooks.slack.com/services/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"; // PUT YOUR WEBHOOK URL HERE
    const data = {
